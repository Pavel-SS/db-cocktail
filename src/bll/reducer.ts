...
export const setItemAC = (items: Array<ItemType>) =>
    ({ type: 'SET_ITEM', items } as const);
type setItemAT = ReturnType<typeof setItemAC>;

export const setLikeAC = (id: string, sorting: boolean) =>
    ({ type: 'SET_LIKE', id, sorting } as const);
type setLikeAT = ReturnType<typeof setLikeAC>;

export const changeFilterAC = (id: string, sorting: boolean) =>
    ({ type: 'CHANGE_TODOLIST_FILTER', id, sorting } as const);
...
export const setItemThunk = async (dispatch: Dispatch): Promise<void> => {
    try {
